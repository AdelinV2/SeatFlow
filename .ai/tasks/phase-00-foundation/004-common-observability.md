# TASK-004: Common Observability Module (GlobalExceptionHandler, CorrelationIdFilter, MDC)
...
Implement the auto-configured `common-observability` shared module. This module provides centralized, standardized HTTP exception handling via `@RestControllerAdvice` and distributed request correlation tracking via `CorrelationIdFilter` and SLF4J MDC.

### Critical Invariants to Enforce:
- [ ] **No service-level `@RestControllerAdvice`:** All exception handling across every microservice is auto-configured exclusively by this module.
...
- [ ] `CorrelationIdFilter` must capture `X-Correlation-Id` from incoming request headers or generate a new UUID, populate MDC (`correlationId`), attach `X-Correlation-Id` to response headers, and cleanly clear MDC in a `finally` block.
...
- `[NEW]` `backend/common/common-observability/src/test/java/com/seatflow/common/observability/filter/CorrelationIdFilterTest.java`
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <description>Auto-configured exception handling, MDC correlation propagation, and tracing helpers</description>
...
### 4.2 Correlation Context & Filter
```java
...
### 4.3 Auto-Configured Global Exception Handler
```java
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleBusinessException(BusinessException ex, HttpServletRequest request) {
        log.warn("Business exception occurred: errorCode={}, message={}", ex.getErrorCode(), ex.getMessage());
...
### 4.4 Spring Boot AutoConfiguration Registration
`CommonObservabilityAutoConfiguration.java`:
```java
package com.seatflow.common.observability.config;

import com.seatflow.common.observability.filter.CorrelationIdFilter;
...
1. **Step 1:** Create `backend/common/common-observability/pom.xml` with dependencies on `common-domain` and `spring-boot-starter-web`.
2. **Step 2:** Implement `CorrelationContext` and `CorrelationIdFilter`.
3. **Step 3:** Implement `GlobalExceptionHandler` with handlers for `BusinessException`, `MethodArgumentNotValidException`, `ConstraintViolationException`, `AccessDeniedException`, and catch-all `Exception`.
4. **Step 4:** Implement `CommonObservabilityAutoConfiguration` and register in `org.springframework.boot.autoconfigure.AutoConfiguration.imports`.
5. **Step 5:** Write unit/slice tests using `@WebMvcTest` with dummy test controllers to verify:
   - Proper mapping of `ResourceNotFoundException` -> 404
   - Proper mapping of `ConflictException` -> 409
   - Proper mapping of `ValidationException` -> 400
   - Proper extraction and formatting of `@Valid` field errors
   - Propagation of `X-Correlation-Id` header to response and `ApiErrorResponse`.
...
```
- [ ] Auto-configuration loads beans into Spring context seamlessly.
- [ ] All exception types map to consistent `ApiErrorResponse` payloads.
- [ ] MDC correlation context is populated and cleaned up per-request.
